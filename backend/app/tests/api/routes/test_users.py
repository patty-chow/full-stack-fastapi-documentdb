...
async def test_get_existing_user_current_user(client: TestClient) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = await crud.create_user(user_create=user_in)
...
async def test_update_user_me(
    ...
    user_db = await crud.get_user_by_email(email=email)
    assert user_db
...
async def test_update_password_me(
    ...
    user_db = await crud.get_user_by_email(email=settings.FIRST_SUPERUSER)
    assert user_db
...
async def test_register_user(client: TestClient) -> None:
    ...
    user_db = await crud.get_user_by_email(email=username)
    assert user_db
...
async def test_delete_user_me(client: TestClient) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = await crud.create_user(user_create=user_in)
    ...
    result = await crud.get_user_by_id(user_id=user_id)
